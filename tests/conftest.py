...
Provides in-memory implementations of MarketDataPort and StoragePort so the
core (Supervisor, StreamActor) can be tested without spinning up MariaDB or
hitting the real Binance API. Real adapters are exercised in their own test
modules with testcontainers / aioresponses.
"""
...
def make_candle(
    *,
    ...
    interval : str
        Candle interval.
    open_time : int
        Open time in ms; ``close_time`` is set to ``open_time + 60_000 - 1``.
    is_closed : bool
    ...
    -------
    Candle
        A candle with constant prices and volumes.
    """
...
class FakeStorage(StoragePort):
    ...
    def __init__(self) -> None:
        """Initialize empty registry and per-key candle buffers."""
    ...
    async def register_stream(self, key: StreamKey, started_at: int) -> StreamInfo:
        """Insert or reactivate a registry entry."""
    ...
    async def mark_stream_stopped(self, key: StreamKey, stopped_at: int) -> None:
        """Mark a registry entry as stopped."""
    ...
    async def get_last_end_ts(self, key: StreamKey) -> int | None:
        """Return the largest close_time stored for ``key``, if any."""
    ...
    async def list_streams(self, status: StreamStatus | None = None) -> list[StreamInfo]:
        """Return the registry entries, optionally filtered by status."""
    ...
    async def read_candles(
        ...
        *,
        ...
        """Return stored candles filtered by close_time, ordered ASC."""
        bucket: list[Candle] = self.candles.get(key, [])
...
class FakeMarket(MarketDataPort):
    ...
    def __init__(self, historical: list[Candle] | None = None) -> None:
        """Store a default historical batch and an asyncio Event for live."""
    ...
    async def fetch_historical(
        ...
        """Yield the preset historical batch (filtered by start/end)."""
    ...
    async def stream_live(self, key: StreamKey) -> AsyncIterator[Candle]:
        """Yield candles produced via :attr:`live_queue` until cancelled."""
        while True:
